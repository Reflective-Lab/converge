...
- [x] Relocate **ferrox** from `~/dev/reflective/mosaic-extensions/ferrox-solvers` to `~/dev/reflective/mosaic-extensions/ferrox`
      to match the extension naming.
- [x] Establish **embassy** as the extension home for source-specific connector
      ports such as LinkedIn (`~/dev/reflective/mosaic-extensions/embassy`, 2026-05-05).
- [x] Decide storage boundary: storage is a port; runtime persistence is
      host-wired; reusable database adapters live in extensions; Runtime Runway owns
...
OR-Tools integration now lives in `~/dev/reflective/mosaic-extensions/ferrox`. Converge pack solvers remain on the
