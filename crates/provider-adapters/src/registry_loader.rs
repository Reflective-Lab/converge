...
//!
//! ```ignore
//! use converge_provider_adapters::registry_loader::{load_registry, RegistryConfig};
//!
...
///
/// ```
/// use converge_provider_adapters::registry_loader::generate_schema;
///
...
pub const DEFAULT_REGISTRY_PATH: &str = "crates/provider-adapters/config/models.yaml";

/// Loads the registry from the default path.
///
/// Tries these paths in order:
/// 1. `crates/provider-adapters/config/models.yaml` (when run from workspace root)
/// 2. `config/models.yaml` (when run from converge-provider-adapters directory)
