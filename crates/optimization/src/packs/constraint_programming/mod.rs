//! Constraint Programming Pack
//!
//! General constraint programming solver using backtracking with constraint propagation.
...
pub struct ConstraintProgrammingPack;
...
impl Pack for ConstraintProgrammingPack {
    ...
    fn validate_inputs(&self, inputs: &serde_json::Value) -> Result<()> {
        let input: ConstraintProgrammingInput = serde_json::from_value(inputs.clone())
    ...
    fn solve(&self, spec: &ProblemSpec) -> Result<PackSolveResult> {
        let input: ConstraintProgrammingInput = spec.inputs_as()?;
        input.validate()?;
        ...
        let confidence = if output.feasible { 0.9 } else { 0.0 };
