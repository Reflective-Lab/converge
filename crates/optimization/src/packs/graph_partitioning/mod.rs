//! Graph Partitioning Pack
//!
//! Balanced graph partitioning minimizing cut weight.
...
pub struct GraphPartitioningPack;

impl Pack for GraphPartitioningPack {
    ...
    fn validate_inputs(&self, inputs: &serde_json::Value) -> Result<()> {
        let input: GraphPartitioningInput = serde_json::from_value(inputs.clone())
    ...
    fn solve(&self, spec: &ProblemSpec) -> Result<PackSolveResult> {
        let input: GraphPartitioningInput = spec.inputs_as()?;
        input.validate()?;

        let solver = GreedyKernighanLinSolver;
